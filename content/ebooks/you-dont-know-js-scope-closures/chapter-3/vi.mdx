...
section: "Chapter 3: Function Versus Block Scope"
chapter: "Chapter 3: Function Versus Block Scope"
slug: you-dont-know-js-scope-closures/chapter-3
date: 2025-04-07
locale: vi
type: child
id: 3
...
As we explored in [Chapter 2](/en/ebooks/you-dont-know-js-scope-closures/chapter-2), scope consists of a series of “bubbles” that each act as a container or bucket, in which identifiers (variables, functions) are declared. These bubbles nest neatly inside each other, and this nesting is defined at author time.
...
There’s a variety of reasons motivating this scope-based hiding. They tend to arise from the software design principle [Principle of Least Privilege](https://en.wikipedia.org/wiki/Principle_of_least_privilege), also sometimes called Least Authority or Least Exposure. This principle states that in the design of software, such as the API for a module/object, you should expose only what is minimally necessary, and “hide” everything else.
...
As such, you can code defensively and achieve the same results as the dependency managers do without actually needing to use them, if you so choose. See the [Chapter 5](/en/ebooks/you-dont-know-js-scope-closures/chapter-5) for more information about the module pattern.
...
className="my-0"
>
<p>The easiest way to distinguish declaration vs. expression is the position of the word `function` in the statement (not just a line, but a distinct statement). If `function` is the very first thing in the statement, then it’s a function declaration. Otherwise, it’s a function expression.</p>
</ImageWithDescription>
</Callout>
...
We pass in the `window` object reference, but we name the parameter global, so that we have a clear stylistic delineation for global versus nonglobal references. Of course, you can pass in anything from an enclosing scope you want, and you can name the parameter(s) anything that suits you. This is mostly just stylistic choice.
...
We learned about with in [Chapter 2](/en/ebooks/you-dont-know-js-scope-closures/chapter-2). While it is a frowned-upon construct, it *is* an example of (a form of) block scope, in that the scope that is created from the object only exists for the lifetime of that `with` statement, and not in the enclosing scope.
...
className="my-0"
>
<div><p>While this behavior has been specified and true of practically all standard JS environments (except perhaps old IE), many linters seem to still complain if you have two or more `catch` clauses in the same scope that each declare their error variable with the same identifier name. This is not actually a redefinition, since the variables are safely block-scoped, but the linters still seem to, annoyingly, complain about this fact.</p><p>To avoid these unnecessary warnings, some devs will name their `catch` variables `err1`, `err2`, etc. Other devs will simply turn off the linting check for duplicate variable names.</p></div>
</ImageWithDescription>
</Callout>
...
In [Chapter 4](/en/ebooks/you-dont-know-js-scope-closures/chapter-4), we will address hoisting, which talks about declarations being taken as existing for the entire scope in which they occur.
...
Another reason block-scoping is useful relates to closures and garbage collection to reclaim memory. We’ll briefly illustrate here, but the closure mechanism is explained in detail in [Chapter 5](/en/ebooks/you-dont-know-js-scope-closures/chapter-5).
...
The reason why this per-iteration binding is interesting will become clear in [Chapter 5](/en/ebooks/you-dont-know-js-scope-closures/chapter-5) when we discuss closures.
